...
result_df = pd.DataFrame(columns=['timestamp', 'Hull number', 'Vertices', 'Hull Indices', 'Edges',
                         'Width', 'Height', 'Area', 'Aspect Ratio of Hull', 'Compactness of Hull', 'Circularity of Hull'])
...
for name, group in groups:
    ...
    for cluster, color in zip(range(num_clusters), colors):
        ...
        temp_df = pd.DataFrame({'timestamp': [name], 'Hull number': [cluster], 'Vertices': [vertices], 'Hull Indices': [hull_indices], 'Edges': [edges], 'Width': [
                               width], 'Height': [height], 'Area': [area], 'Aspect Ratio of Hull': [aspect_ratio], 'Compactness of Hull': [compactness], 'Circularity of Hull': [circularity]})
        result_df = pd.concat([result_df, temp_df], ignore_index=True)
        # Plot the points in the cluster
        plt.scatter(-cluster_points[:, 0], cluster_points[:, 1],
                    color=color, s=50, label='Cluster '+str(cluster))
        # Mark the indices of the points on the convex hull
        for i in hull_indices:
            if cluster_points.shape[0] == 0:
                continue
            plt.text(-cluster_points[i, 0], cluster_points[i,
                     1], str(i), color='b', fontsize=10)
        # Plot the convex hull around the cluster
        for simplex in hull.simplices:
            if cluster_points.shape[0] == 0:
                continue
            plt.plot(-cluster_points[simplex, 0],
                     cluster_points[simplex, 1], 'k-')
